...
bool init_gui_params(GUI::GUI_InitParams &gui_params, int argc, char **argv, Data &cli)
{
    ...
    {
        // On Unix systems, the preFlight binary may be symlinked to give the application a different meaning.
        gui_params.start_as_gcodeviewer =
    ...
    if (cli.misc_config.has("opengl-version"))
    {
        ...
        if (semver.has_value() && (*semver) >= opengl_minimum)
        {
            ...
            boost::nowide::cerr << "Required OpenGL version " << opengl_version_str
                                << " is invalid. Must be greater than or equal to " << opengl_minimum.to_string()
                                << "\n Option 'opengl-version' ignored." << std::endl;
        start_gui = true;
    }
    ...
    if (!gui_params.start_as_gcodeviewer && !cli.input_config.has("load"))
    {
        ...
        if (cli.input_files.size() == 1 && is_gcode_file(cli.input_files[0]) &&
            boost::filesystem::exists(cli.input_files[0]))
            gui_params.start_as_gcodeviewer = true;
    }
...
int start_as_gcode_viewer(GUI::GUI_InitParams &gui_params)
{
    ...
    if (!gui_params.input_files.empty())
    {
        ...
        {
            boost::nowide::cerr << "Input file isn't a .gcode file or doesn't exist. GCodeViewer can't be start."
                                << std::endl;
            return 1;
        }
    }
