...
static opts_map get_opts_map(const Data &data)
{
    ...
    for (const CLI_DynamicPrintConfig *config :
         {&data.input_config, &data.overrides_config, &data.transform_config, &data.misc_config, &data.actions_config})
    {
...
{
    for (CLI_DynamicPrintConfig *config :
         {&data.input_config, &data.overrides_config, &data.transform_config, &data.misc_config, &data.actions_config})
    {
        if (type == config->type())
            return config;
...
static bool read(Data &data, int argc, const char *const argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        ConfigOptionVectorBase *opt_vector = opt_base->is_vector() ? static_cast<ConfigOptionVectorBase *>(opt_base)
                                                                   : nullptr;
        if (opt_vector)
        {
            if (!existing)
...
static bool setup_common()
{
    ...
    {
        std::wstring text = L"Following DLLs have been injected into the preFlight process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"preFlight is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using preFlight.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes the application "
                L"crash on a secondary monitor.";
        MessageBoxW(NULL, text.c_str(), L"Warning" /*L"Incopatible library found"*/, MB_OK);
    }
#endif

    // See: invoking from $PATH environment variable can cause crashes
